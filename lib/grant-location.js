...
function looksLikePrivateCitySlug(citySlug, titleSlug) {
...
function inferGrantLocation(grant) {
  const rawJurisdiction = (grant && grant.jurisdiction) || null;
  const baseType = (grant && grant.base_type) || null;
  const inferredType = (grant && grant.type) || null;
  ...
  const titleSlug = slugify((grant && typeof grant.title === "string" ? grant.title : "") || "");
