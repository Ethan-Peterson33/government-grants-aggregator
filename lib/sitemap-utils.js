...
async function fetchStatesWithGrants() {
  ...
  try {
    ...
    const aggregated = aggregateStateFacetsFromSource(data ?? []);
    console.log("[sitemap-utils] Aggregated states for sitemap", {
      rawCount: Array.isArray(data) ? data.length : null,
      aggregatedCount: aggregated.length,
      sample: aggregated.slice(0, 5),
    });
    if (!aggregated.length) {
      console.warn("[sitemap-utils] No states discovered for sitemap generation");
    }
    return aggregated;
  } catch (error) {
...
async function fetchAgencyRecords() {
  ...
  if (!supabase) {
    console.error(
      "[sitemap-utils] Supabase client unavailable for agency sitemap generation"
    );
    return [];
  }

  try {
    // Start from grants and join agencies via agency_id
    const { data, error } = await supabase
      .from("grants")
      .select(`
        agency_id,
    ...
    for (const raw of data) {
      const agencyRow = raw.agencies;
      if (!agencyRow) continue;

      const slug = deriveAgencySlug({
        agency_code: agencyRow.agency_code ?? undefined,
        agency_name: agencyRow.agency_name ?? undefined,
        agency: agencyRow.agency_name ?? undefined,
      });

      if (!slug) continue;

      // Use the canonical agency.id as the key to avoid collisions
      const key = agencyRow.id;

      if (!agencies.has(key)) {
        agencies.set(key, {
          id: agencyRow.id,
          slug,
          agency_id: raw.agency_id ?? agencyRow.id,
          agency_code:
            typeof agencyRow.agency_code === "string"
              ? agencyRow.agency_code
              : null,
          agency_name:
            typeof agencyRow.agency_name === "string"
              ? agencyRow.agency_name
              : null,
          // keep `agency` field if your downstream code expects it
          agency:
            typeof agencyRow.agency_name === "string"
              ? agencyRow.agency_name
              : null,
    ...
    console.error(
      "[sitemap-utils] Exception fetching agencies for sitemap",
      error
    );
    return [];
  }
...
async function getStateUrls(siteUrl = DEFAULT_SITE_URL, options = {}) {
  ...
  console.log("[sitemap-utils] Building state index urls", {
    source: grantSource ? "grants" : "supabase",
...
async function fetchCategoryStateCombos() {
  const supabase = createServerSupabaseClient();
  if (!supabase) {
    console.error("[sitemap-utils] Supabase client unavailable for category+state sitemap generation");
    return [];
  }
  ...
  try {
    const { data, error } = await supabase
      .from("grants")
      .select("state, category_code, active, grant_categories!inner(slug)")
      .not("state", "is", null)
      .neq("state", "")
    ...
    if (error) {
      console.error("[sitemap-utils] Error fetching category/state combos for sitemap", error);
      return [];
    }

    return Array.isArray(data) ? data : [];
  } catch (error) {
    console.error("[sitemap-utils] Exception fetching category/state combos for sitemap", error);
    return [];
  }
}

async function getCategoryStateUrls(siteUrl = DEFAULT_SITE_URL) {
  const combos = await fetchCategoryStateCombos();
  if (!combos.length) return [];

  const entries = new Map();

  for (const combo of combos) {
    const categorySlug = combo?.grant_categories?.slug;
    const rawState = combo?.state;
    if (!categorySlug || typeof rawState !== "string") continue;

    const stateInfo = findStateInfo(rawState);
    const stateSegment = toStatePathSegment(stateInfo?.code ?? rawState);
    if (!stateSegment) continue;

    const path = `/grants/category/${encodeURIComponent(categorySlug)}/${encodeURIComponent(stateSegment)}`;
    if (!entries.has(path)) {
      entries.set(
        path,
        createEntry(path, siteUrl, {
          changefreq: "weekly",
          priority: 0.52,
        })
      );
  ...
  console.log("[sitemap-utils] Category+state sitemap urls", {
    comboCount: combos.length,
    entryCount: entries.size,
    sample: Array.from(entries.keys()).slice(0, 5),
  });
...
async function getAllSitemapUrls(siteUrl = DEFAULT_SITE_URL) {
  ...
  const stateGrantSections = await Promise.all(
    (states ?? []).map((state) =>
      collectGrantEntriesForSitemap({
        ...
        requireState: true,
        state: state.code,
        entryOverrides: { jurisdiction: "state" },
      })
    )
  );

  console.log("[sitemap-utils] Completed state grant sitemap fetches", {
    stateCount: states?.length ?? 0,
    stateSamples: states?.slice(0, 5) ?? [],
    sectionSamples: stateGrantSections
      .slice(0, 3)
      .map((section) => ({
        state: section?.grants?.[0]?.state ?? null,
        grantCount: section?.grants?.length ?? 0,
      })),
  });
  ...
  console.log("[sitemap-utils] Aggregated sitemap urls", {
    ...
    categoryStateCount: categoryStateUrls.length,
...
async function getAllGrantPaths(siteUrl = DEFAULT_SITE_URL) {
  ...
  console.log("[sitemap-utils] State grant sitemap path generation", {
    stateCount: states?.length ?? 0,
    stateSamples: states?.slice(0, 5) ?? [],
    sectionSamples: stateGrantSections
      .slice(0, 3)
