...
async function fetchStatesWithGrants() {
  ...
  try {
    ...
    for (const row of data ?? []) {
      const facet = toStateFacet(row?.state);
      if (!facet) continue;
...
async function getGrantUrls(siteUrl = DEFAULT_SITE_URL) {
  const grants = await fetchGrantsForSitemap({ excludeJurisdictions: ["private", "federal"] });
...
async function getFederalGrantUrls(siteUrl = DEFAULT_SITE_URL) {
  const grants = await fetchGrantsForSitemap({ jurisdiction: "federal" });
  if (!grants.length) return [];

  console.log("[sitemap-utils] Building federal grant sitemap entries", {
    grantCount: grants.length,
    sample: grants.slice(0, 3).map((grant) => ({
      id: grant.id,
      title: grant.title,
  ...
  console.log("[sitemap-utils] Sample federal grant sitemap paths", {
    total: entries.length,
    sample: entries.slice(0, 3).map((entry) => entry.path),
  });
...
async function getAllSitemapUrls(siteUrl = DEFAULT_SITE_URL) {
  const [grantUrls, federalGrantUrls, privateGrantUrls, agencyUrls, categoryUrls, stateUrls] =
    await Promise.all([
      getGrantUrls(siteUrl),
      getFederalGrantUrls(siteUrl),
  ...
  console.log("[sitemap-utils] Aggregated sitemap urls", {
    ...
    federalGrantCount: federalGrantUrls.length,
...
async function getAllGrantPaths(siteUrl = DEFAULT_SITE_URL) {
  const [grantUrls, federalGrantUrls, privateGrantUrls] = await Promise.all([
    getGrantUrls(siteUrl),
    getFederalGrantUrls(siteUrl),
    getPrivateGrantUrls(siteUrl),
  ]);
  return [...grantUrls, ...federalGrantUrls, ...privateGrantUrls].map((entry) => entry.path);
}
