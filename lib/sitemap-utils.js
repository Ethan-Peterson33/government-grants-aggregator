...
const AGENCY_JOIN_ALIAS = "agency_record";
const AGENCY_JOIN_SELECT = `${AGENCY_JOIN_ALIAS}:agencies!grants_agency_id_fkey (
  id,
  agency_code,
  agency_name,
  type,
...
async function fetchAgencyRecords() {
  ...
  try {
    const { data, error } = await supabase
      .from("grants")
      .select("agency_code, agency_name, agency, agency_slug")
      .or("active.is.null,active.eq.true");
...
function extractAgencyJoin(grant) {
  if (!grant || typeof grant !== "object") return null;
  const join = grant?.[AGENCY_JOIN_ALIAS] ?? grant?.agencies ?? null;
  if (Array.isArray(join)) {
    return join.find((entry) => entry && typeof entry === "object") || null;
  }
  return join && typeof join === "object" ? join : null;
}

function normalizeGrantForPath(grant, options = {}) {
  if (!grant || typeof grant !== "object") return grant;
  const normalized = { ...grant };
  const agencyJoin = extractAgencyJoin(grant);

  if (agencyJoin) {
    if (!normalized.agency_code && typeof agencyJoin.agency_code === "string") {
      normalized.agency_code = agencyJoin.agency_code;
    }
    if (!normalized.agency_name && typeof agencyJoin.agency_name === "string") {
      normalized.agency_name = agencyJoin.agency_name;
    }
    if (!normalized.agency && typeof agencyJoin.agency_name === "string") {
      normalized.agency = agencyJoin.agency_name;
    }
    if (!normalized.type && typeof agencyJoin.type === "string") {
      normalized.type = agencyJoin.type;
    }
    if (!normalized.base_type && typeof agencyJoin.base_type === "string") {
      normalized.base_type = agencyJoin.base_type;
    }
  }

  if (typeof options.jurisdiction === "string" && options.jurisdiction) {
    normalized.jurisdiction = options.jurisdiction;
  }
...
function buildGrantEntries(grants, siteUrl = DEFAULT_SITE_URL, overrides = {}) {
  ...
  return grants
    .map((grant) => {
      try {
        const normalizedGrant = normalizeGrantForPath(grant, {
...
async function collectGrantEntriesForSitemap(options = {}) {
  ...
  const grants = await fetchGrantsForSitemap({ jurisdiction, excludeJurisdictions });
  if (!grants.length) {
...
async function getGrantUrls(siteUrl = DEFAULT_SITE_URL) {
  const { entries } = await collectGrantEntriesForSitemap({
    siteUrl,
    excludeJurisdictions: [PRIVATE_JURISDICTION, FEDERAL_JURISDICTION],
  });
  return entries;
}
...
async function getStateUrls(siteUrl = DEFAULT_SITE_URL, options = {}) {
  ...
  const states = grantSource
    ? aggregateStateFacetsFromSource(grantSource)
    : await fetchStatesWithGrants();
...
async function getAllSitemapUrls(siteUrl = DEFAULT_SITE_URL) {
  const [stateGrantSection, federalGrantSection, privateGrantSection, agencyUrls, categoryUrls] =
    await Promise.all([
  ...
  const stateUrls = await getStateUrls(siteUrl, { grants: stateGrantSection.grants });
  ...
  console.log("[sitemap-utils] Aggregated sitemap urls", {
    ...
    stateGrantCount: stateGrantSection.entries.length,
...
async function getAllGrantPaths(siteUrl = DEFAULT_SITE_URL) {
  const [stateGrantSection, federalGrantSection, privateGrantSection] = await Promise.all([
    collectGrantEntriesForSitemap({
      siteUrl,
      excludeJurisdictions: [PRIVATE_JURISDICTION, FEDERAL_JURISDICTION],
    }),
  ...
  return [...stateGrantSection.entries, ...federalGrantSection.entries, ...privateGrantSection.entries].map(
    (entry) => entry.path
  );
}
