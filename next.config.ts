...
const nextConfig: NextConfig = {
  ...
  async redirects() {
    return [
      ...
      {
        source: "/grants/:state((?!federal|state|local|category|private).+)/statewide/:category/:slug",
        destination: "/grants/state/:state/:slug",
        permanent: true,
      },
      {
        source: "/grants/:state((?!federal|state|local|category|private).+)/statewide/:category",
        destination: "/grants/state/:state?category=:category",
        permanent: true,
      },
      {
        source: "/grants/:state((?!federal|state|local|category|private).+)/statewide",
        destination: "/grants/state/:state",
        permanent: true,
      },
      {
        source: "/grants/:state((?!federal|state|local|category|private).+)/:city/:category/:slug",
        destination: "/grants/local/:state/:city/:slug",
        permanent: true,
      },
      {
        source: "/grants/:state((?!federal|state|local|category|private).+)/:city/:category",
        destination: "/grants/local/:state/:city?category=:category",
        permanent: true,
      },
      {
        source: "/grants/:state((?!federal|state|local|category|private).+)/:city",
        destination: "/grants/local/:state/:city",
        permanent: true,
      },
      {
        source: "/grants/:state((?!federal|state|local|category|private).+)",
        destination: "/grants/state/:state",
        permanent: true,
      },
    ];
