...
export async function generateMetadata({
  ...
  return {
    title: `${cityName}, ${stateName} Government Jobs`,
    description: `Discover open government roles located in ${cityName}, ${stateName}, including city, county, and regional agencies.`,
  };
}
...
export default async function CityJobsPage({
  ...
  const { jobs, total } = await searchJobs({ location: locationFilter, page, pageSize: PAGE_SIZE });
    ...
    <div className="container-grid space-y-6 py-10">
      ...
      <header className="space-y-2">
        <h1 className="text-3xl font-semibold text-slate-900">
          {cityName} government jobs
        </h1>
        <p className="text-slate-600">
          Public sector employers in {cityName} are hiring now. Review job postings across departments and nearby jurisdictions.
        </p>
      </header>
          ...
          {jobs.length ? (
            ...
            <Card>
              <CardContent className="py-8 text-center text-sm text-slate-600">
                <p>No jobs here just yet. Check nearby cities or broaden your filters.</p>
              </CardContent>
            </Card>
          )}
        </div>
      ...
      <RelatedLinks
        links={[
          { label: `More jobs across ${stateName}`, href: `/jobs/${state}` },
          { label: `Public safety roles in ${cityName}`, href: `/jobs/${state}/${city}/public-safety` },
        ]}
      />
